...
## Project Notes
This repository is based on the original `whisper-diarization` work and the local developer setup in [`README_DEVELOPER.md`](README_DEVELOPER.md).

This version adds practical enhancements focused on easier day-to-day usage:
- Audio preprocessing before transcription/diarization (source separation, normalization, and conversion flow)
- A simpler one-command execution path through `diarize.py`

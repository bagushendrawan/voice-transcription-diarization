...
def write_html(transcript, file, title="Transcript"):
    """
    Write a transcript to a readable HTML format.
    """
...
""",
    ...
    for i, segment in enumerate(transcript, start=1):
        ...
        speaker = esc(segment.get("speaker", "Speaker"))
        text = esc(segment["text"].strip().replace("-->", "->"))
